...
/**
 * "Shop the story" picker for the Journal — same picker as products' own
 * "frequently bought together" widget (a button opens a modal listing
 * every product to check on/off, current selection summarized underneath),
 * copied rather than shared per this codebase's convention for small
 * per-feature admin widgets (see CollectionRowActions vs ProductRowActions).
 * Emits `relatedProductSlugs` hidden inputs, matching the
 * `related_product_slugs` column on both products and posts.
 */
export default function RelatedProductsPicker({
  ...
  return (
    <div>
      ...
      <Modal open={open} onClose={() => setOpen(false)} title="Related products">
