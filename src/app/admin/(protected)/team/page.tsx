...
function statusLabel(entry: { hasAccount: boolean; totpEnabled: boolean; disabled: boolean }) {
  if (!entry.hasAccount) return { text: "Approved — not signed up yet", cls: "bg-slate-100 text-slate-600" };
  if (entry.disabled) return { text: "Disabled", cls: "bg-red-50 text-red-700" };
  if (!entry.totpEnabled) return { text: "Signed up — 2FA incomplete", cls: "bg-amber-50 text-amber-700" };
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center justify-end gap-2">
                          {user && (
                            <form action={setUserDisabledAction.bind(null, user.id, !user.disabled)}>
                              <button
                                ...
                                className="rounded-full border border-slate-300 px-3 py-1.5 text-xs font-semibold text-slate-700 transition-colors hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-40"
                          ...
                          <form action={removeEmailAction.bind(null, entry.email)}>
                            <button
                              type="submit"
                              className="rounded-full border border-slate-300 px-3 py-1.5 text-xs font-semibold text-slate-500 transition-colors hover:bg-slate-50"
                            >
                              Remove
                            </button>
                          </form>
